...
try {
    ...
    foreach ($rate in $response.data) {
        Write-Host "  - $($rate.vehicle_type_display): Rs$($rate.hourly_rate)/hr, Rs$($rate.daily_rate)/day" -ForegroundColor White
    }
} catch {
...
try {
    ...
    Write-Host "SUCCESS! Total Fee: Rs$($response.data.total_fee)" -ForegroundColor Green
    Write-Host "  Rate Plan: $($response.data.rate_name)" -ForegroundColor White
    Write-Host "  Hourly Rate: Rs$($response.data.hourly_rate)/hr" -ForegroundColor White
} catch {
    Write-Host "FAILED: $_" -ForegroundColor Red
}
...
try {
    ...
    $response = Invoke-RestMethod -Uri "$baseUrl/rates/calculate-fee/" -Method POST -Body $body -ContentType "application/json"
    Write-Host "SUCCESS! Total Fee: Rs$($response.data.total_fee)" -ForegroundColor Green
    Write-Host "  Daily Rate: Rs$($response.data.daily_rate)/day" -ForegroundColor White
} catch {
    Write-Host "FAILED: $_" -ForegroundColor Red
}
...
Write-Host "All public API endpoints tested!" -ForegroundColor Green
Write-Host ""
